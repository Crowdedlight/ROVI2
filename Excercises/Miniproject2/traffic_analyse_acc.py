...
def backgroundSubtraction(frame):
    kernel_open = cv2.getTrackbarPos('opening', 'masked_frame')
    kernel_close = cv2.getTrackbarPos('closing', 'masked_frame')
...
def thresholdComponents(stats, centroids):
    ...
    for i in range(0, len(centroids)):
        ...
        thresComponent.append((l, t, w, h, 0))
...
cv2.namedWindow("tracked_frame", cv2.WINDOW_NORMAL)

cv2.createTrackbar('cc_min','masked_frame',0,3000, nothing)
cv2.createTrackbar('opening','masked_frame',1,100, nothing)
cv2.createTrackbar('closing','masked_frame',1,100, nothing)

# Init pos
cv2.setTrackbarPos('cc_min', 'masked_frame', 38) #38
cv2.setTrackbarPos('opening', 'masked_frame', 2) #
cv2.setTrackbarPos('closing', 'masked_frame', 4) #
...
while cap.isOpened():
    ...
    # Get threshold
    cc_min = cv2.getTrackbarPos('cc_min', 'masked_frame')
    ...
    cv2.imshow('masked_frame', foregroundFrame)
    cv2.imshow('tracked_frame', outFrame)
...
# show result - Due to PyCharm newest IDE update use Qt5 rendering to get extended features in matplotlib
import matplotlib
